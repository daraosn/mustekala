var Mustekala=function(setup) {
	...
	m.connect=function(connectListener) {
		...
		m.socket=io.connect('http://localhost:3001/mustekala'); // TODO: this must be provided by the app configuration on setup?
	...
	m.trigger=function(password, channel, action, data) {
		if(m.connected) {
			if(m.debug) console.log('m.trigger', 'WARNING: This is for testing purposes only, do not use on production!');
	...
	m.onConnect=function(listener) {
		...
		if(m.debug>1) console.log('m.onConnect');
	}
	m.onDisconnect=function(listener) {
		m.addEventListener('disconnect', listener);
		if(m.debug>1) console.log('m.onDisconnect');
	}
	...
	m.triggerEvent=function() {
		if(m.debug) console.log('m.triggerEvent',m.triggerEvent.arguments);
	...
	function MustekalaChannel(channelName) {
		return {
			...
			,onSubscribe: function(handler) {
				if(m.debug>1) console.log('m.subscribe().channel<'+channelName+'>.onSubscribe');
				m.addEventListener('_'+channelName+':subscribe', handler);
			}
			,on: function(action, handler) {
				if(m.debug>1) console.log('m.subscribe().channel<'+channelName+'>.on', action);
	...
	function MustekalaPresenceChannel(channelName) {
		...
		m.addEventListener('_'+channelName+':memberList', function(members, user) {
			...
			channel.members.me=user;
		});
		channel.onMemberJoin=function(handler) {
			if(m.debug>1) console.log('m.subscribe().presenceChannel<'+channelName+'>.onMemberJoin', handler);
		...
		channel.onMemberLeave=function(handler) {
			if(m.debug>1) console.log('m.subscribe().presenceChannel<'+channelName+'>.onMemberLeave', handler);
