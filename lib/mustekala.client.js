var Mustekala=function(setup) {
	...
	m.subscribe=function(channel) {
		if(m.connected) {
			...
			console.error('Mustekala is not connected.');
		}
	}
	// TODO: m.disconnet()
	/*
	...
	m.triggerEvent=function() {
		...
		if(m.events[eventName]) {
			...
			for(var i in m.events[eventName]) {
				try { // should use try/catch?
					m.events[eventName][i].apply(m,args);
				} catch(e) {
	...
	function MustekalaChannel(channelName) {
		return {
			...
			,on: function(action, handler) {
				if(m.debug) console.log('m.subscribe().channel<'+channelName+'>.on', action);
	...
	function MustekalaPresenceChannel(channelName) {
		...
		channel.onMemberJoin=function(handler) {
			...
			m.addEventListener('_'+channelName+'.memberJoin', handler);
		}
		channel.onMemberLeave=function(handler) {
			...
			m.addEventListener('_'+channelName+'.memberLeave', handler);
		}
		return channel;
	}
