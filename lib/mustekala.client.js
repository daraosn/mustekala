var Mustekala=function(setup) {
	...
	m.connect=function(connectListener) {
		...
		m.socket.on('presence.memberJoin', function(channel, user) {
			// not triggering global event
			console.log('presence.memberJoin');
			
			var key='_'+channel+'.memberJoin';
			if(m.events[key])
				m.triggerEvent(key, user);
			console.log('presence.memberJoin', key)
		});
	}
	...
	function MustekalaChannel(channelName) {
		return {
			...
			,on: function(action, handler) {
				if(m.debug) {
					console.log('m.subscribe().channel<'+channelName+'>.on', action);
				}
				var key='_'+channelName+'.'+action;
				if(!m.events[key])
					m.events[key]=[];
	...
	function MustekalaPresenceChannel(channelName) {
		...
		channel.onMemberJoin=function(handler) {
			var key='_'+channelName+'.memberJoin';
			if(!m.events[key])
				m.events[key]=[];
			m.events[key].push(function(user) {
				handler(user);
			});
		}
		return channel;
